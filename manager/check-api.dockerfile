...
COPY . .
WORKDIR /app/proxy/manager
RUN --mount=type=cache,mode=0755,target=/root/.cache/go-build --mount=type=cache,mode=0755,target=/root/go go build -o /go/bin/proxy_api_checker ./cmd/check_api/main.go

FROM alpine
COPY --from=app-build /go/bin/proxy_api_checker /app/proxy/proxy_api_checker
COPY manager/config.yml /app/proxy/config.yml
CMD ["/app/proxy/proxy_api_checker", "-c", "/app/proxy/config.yml", "-s", "manager-server:8082"]